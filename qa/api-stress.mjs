...
await scenario('over-quota rejects the third seat', async () => {
  ...
  assert.equal((await pickSeat(QA_TOKEN, block.theaterId, first)).ok, true);
  assert.equal((await pickSeat(QA_TOKEN, block.theaterId, second)).ok, true);
  const overflow = await pickSeat(QA_TOKEN, block.theaterId, third);
  assert.equal(overflow.ok, false);
  assert.match(JSON.stringify(overflow.body), /full|placed|seat/i);
  return { theaterId: block.theaterId, seats: block.seatIds, overflowStatus: overflow.status };
});

await scenario('orphan-gap selection is rejected', async () => {
  const pair = await findOrphanPair();
  const first = await pickSeat(QA_TOKEN, pair.theaterId, pair.first);
  assert.equal(first.ok, true);
  const orphan = await pickSeat(QA_TOKEN, pair.theaterId, pair.second);
  assert.equal(orphan.ok, false);
  assert.match(JSON.stringify(orphan.body), /orphan|alone|single|empty|row/i);
  return { theaterId: pair.theaterId, row: pair.row, first: pair.first, rejected: pair.second, status: orphan.status };
});
...
await scenario('stale-tab over-placement is rejected', async () => {
  ...
  const block = await findSeatBlock({ count: 3 });
  assert.equal((await pickSeat(QA_TOKEN, block.theaterId, block.seatIds[0])).ok, true);
  assert.equal((await pickSeat(QA_TOKEN, block.theaterId, block.seatIds[1])).ok, true);
  const stalePick = await pickSeat(QA_TOKEN, block.theaterId, block.seatIds[2]);
...
await scenario('two clients racing for one seat do not create duplicates', async () => {
  ...
  const [a, b] = await Promise.all([
    pickSeat(tokenA, block.theaterId, block.seatIds[0]),
    pickSeat(tokenB, block.theaterId, block.seatIds[0]),
  ]);
  assert.equal(a.ok || b.ok, true);
  const portal = await getPortal(tokenA);
  const matches = [...(portal.myAssignments || []), ...(portal.myHolds || [])].filter(
    (row) => row.theater_id === block.theaterId && `${row.row_label}-${row.seat_num}` === block.seatIds[0]
  );
