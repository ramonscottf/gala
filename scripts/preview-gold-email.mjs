// scripts/preview-gold-email.mjs
//
// Renders the canonical Gold-open email exactly as it'll arrive in Sherry's
// approval review. Wraps the body in galaEmailHtml() and writes the result
// to /mnt/user-data/outputs/ for visual inspection.

import { readFileSync, writeFileSync } from 'node:fs';
import { GOLD_BODY } from './tier-open-email-bodies.mjs';

// Inline the wrapper from functions/api/gala/_notify.js so we don't have
// to spin up the worker. Keep this in sync — galaEmailHtml is the
// canonical wrapper used by marketing-send-now.js.
...
// Substitute a fake token so the preview "Make my selections" button links
// somewhere realistic-looking. Real sends substitute the per-recipient
// rsvp_token at marketing-send-now.js time.
const bodyWithToken = GOLD_BODY.replace(/\{TOKEN\}/g, 'previewtoken1234');
const html = galaEmailHtml({ firstName: 'Mike', body: bodyWithToken });

writeFileSync('/mnt/user-data/outputs/gold-email-preview.html', html);
console.log('Wrote /mnt/user-data/outputs/gold-email-preview.html');
console.log('Body length:', GOLD_BODY.length, 'chars');
